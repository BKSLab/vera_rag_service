...
| Размерность вектора | **768** | Максимальная доступная в v2. Корпус небольшой, рост хранилища некритичен — приоритет точности на тонких смысловых различиях юридических текстов (например, разные статьи закона с похожей формулировкой) |
...
| Reranker | Не финализирован — открытый вопрос | Self-hosted `bge-reranker-base` тоже требует CPU/RAM ресурсов на сервере. Yandex Cloud Model Gallery не предоставляет reranker как услугу. Альтернативы: облачный rerank API (Cohere/Voyage) либо отказ от отдельного reranker-шага на MVP в пользу hybrid search + RRF |
...
### Этап 8 — Observability и логирование запросов
...
**Персистентное логирование поисковых запросов** (отдельно от трейсов Phoenix — те предназначены для инженерной отладки и не гарантируют долгосрочное хранение):
- Хранилище: таблица в Postgres (или другая БД, доступная в инфраструктуре проекта) — не Qdrant, чтобы не смешивать операционные данные поиска с векторным хранилищем
...
| Embedding через внешний API (Yandex) — сетевая зависимость в hot path поиска | Риск задержки/недоступности влияет на latency `embed_query` и целевой SLA "первый токен ≤5 сек" (WBS 5.3) | Таймауты + retry с backoff, мониторинг latency через Phoenix (span `embed_query`) |
