...
- `app/search/hybrid.py`: вместо одного плоского top-20 (`DENSE_TOP_K`/`SPARSE_TOP_K`) — параллельный dense+sparse поиск **на каждую категорию отдельно** (фильтр по `category`, по аналогии с уже существующим фильтром по `audience`/`topic`), меньший top-k на категорию (например, top-8 dense + top-8 sparse), чтобы суммарный объём кандидатов на входе RRF/reranker не вырос кратно. Категория без кандидатов после фильтра просто не даёт вклада — без ошибки, по аналогии с деградацией reranker'а при отказе LLM (Этап 6).
...
### Этап 11 — Админка: управление документами и интерактивное тестирование поиска
> Зафиксировано 2026-06-20 по итогам ревью Этапа 8 (админка с журналом `search_logs`) — найден разрыв между тем, как документы реально готовит Expert, и тем, что умеет принимать сервис. Не реализовано, оба пункта — следующий приоритет после Этапов 9–10.

**11.1. Загрузка и управление документами через `/admin`**
- **Разрыв в текущей реализации:** `POST /ingest` принимает `raw_text` — текст документа, уже декодированный в строку (`app/models/schemas.py::IngestRequest`). Приёма файлов (PDF/MD/TXT) нигде нет: ни на API, ни отдельным скриптом — `scripts/ingest_corpus.py`, упомянутый в разделе 1 (структура проекта) и Этапе 4 как CLI для пакетной загрузки, **не существует**, это была только заявка в документации. Expert (раздел 0 плана: готовит нормативные акты и авторские статьи) физически не может загрузить документ без того, чтобы кто-то технический вручную собрал JSON с полным текстом внутри строки.
- **Решение:** полный цикл операций с документами (загрузка файла → извлечение текста → запуск ingestion с заполнением метаданных документа → просмотр списка документов в БЗ → удаление / запуск новой редакции) переносится в `/admin` — Expert и контент-менеджер не должны работать с API напрямую.
- **Открытый вопрос, требует решения до реализации:** откуда админка берёт список документов для отображения/удаления — нет ни одной таблицы в Postgres, описывающей документ как сущность (есть только `search_logs`, журнал запросов, не документов). Два варианта:
  - (а) новая таблица `documents` в Postgres — пишется при каждом успешном `ingest_document` (document_id, version, source_type, source_title, audience, topic, effective_date, is_active, created_at); список/удаление в админке — обычный `ModelView` (раздел 14 `FASTAPI_PATTERNS.md`), история версий видна даже после удаления чанков из Qdrant;
  - (б) без новой таблицы — список строится на лету через `scroll` по уникальным `document_id` прямо в Qdrant; проще по объёму кода, но требует кастомного `BaseView` (sqladmin не умеет рисовать `ModelView` над не-SQL источником) и не даёт истории версий после удаления.
- Извлечение текста из PDF/MD/TXT — новая зависимость (например, `pdfplumber` для PDF), отдельный модуль (по аналогии с `app/ingestion/preprocess.py`, но это отдельный шаг до препроцессинга — "декодирование файла в строку", не "извлечение структуры").
- Форма в админке — кроме самого файла, все поля `IngestRequest`, кроме `raw_text`: `document_id`, `source_type`, `source_title`, `audience`, `topic`, `version`, `effective_date`.

**11.2. Интерактивное тестирование поиска через `/admin`**
- **Назначение:** тестировщик, контент-мейкер и юрист должны иметь возможность задать вопрос (как реальный пользователь Веры) и увидеть, как RAG Service готовит ответ — какие чанки нашлись на каждой стадии (dense/sparse top-20 → RRF → reranker top-5) и что отдаётся финальным результатом — **без необходимости поднимать Agent Service/MCP Tools Server и без обращения к API через Postman/curl**.
- Граница ответственности (см. Этап 8 выше) не меняется: показывается retrieval-результат RAG Service (топ-5 чанков с метаданными), не финальный текстовый ответ Веры — генерация ответа агентом сюда не входит и не подменяется.
- Технически — новая страница в админке (`BaseView` + `@expose`, по образцу `UserFavoritesView`/`StatsView` из `api_work_for_everyone/app/admin/views.py`): форма (текст вопроса + опциональные фильтры `audience`/`topic`/`source_type`) → вызывает `SearchService.search` напрямую (тот же сервис, что и `POST /search`, без дублирования логики) → рендерит результат по стадиям: dense top-20, sparse/BM25 top-20, RRF-склейка, итог reranker'а, финальный список с `score`.
- Так как `SearchService.search` уже сам пишет запись в `search_logs` (Этап 8), каждый такой тестовый прогон через админку автоматически попадает в журнал — дополнительной логики логирования не нужно.
