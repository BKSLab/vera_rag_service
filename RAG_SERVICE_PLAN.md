...
> - 📋 **Задача (зафиксировано 2026-06-24, не реализовано): проверить возможность увеличения размерности эмбеддинга с 256 до 768.** Низкая размерность снижает разрешающую способность dense-поиска именно там, где это болезненнее всего для этого корпуса — юридический текст намеренно повторяющийся («работодатель обязан», «в соответствии с настоящим Федеральным законом»), статьи различаются часто только номером/одной деталью условия (раздел 0.1, "Особенности поиска по юридическим текстам"); 256 измерений хуже разводят такие близкие по лексике, но разные по смыслу пары, чем 768 — выше риск, что нужный чанк не попадёт даже в top-20 кандидатов dense-поиска. Критичность смягчена (не блокер) тем, что dense-поиск — не единственный фильтр: sparse-поиск (точные термины типа "статья 21") и финальный LLM-reranker не зависят от размерности dense-вектора и компенсируют часть просадки recall. Действия: (1) уточнить у Yandex Cloud / в актуальной документации, не появился ли отдельный параметр запроса для увеличения размерности v2-модели; (2) если нет — оценить миграцию на другую embedding-модель с поддержкой 768+ при сохранении требования о юрисдикции РФ (152-ФЗ, см. раздел 0.1); (3) если решение об увеличении размерности принято — потребует полной переиндексации всего корпуса (повторная оплата LLM-обогащения и embedding для всех документов, не только дельта). Не реализуется до получения ответа от Yandex/решения по альтернативной модели.
...
> - 📋 Reranker как арбитр релевантности — сигнал "нет ответа" (зафиксировано 2026-06-25, не реализовано). Сейчас если reranker возвращает пустой список кандидатов, код деградирует до RRF-порядка вместо того, чтобы сигнализировать "ничего релевантного не найдено". Кроме того, `RerankResult.ranked_indices` имеет `min_length=1` — модель физически не может вернуть пустой список (Pydantic не пропустит). Цена ошибки на этом проекте высока (права людей с инвалидностью), поэтому возврат нерелевантных чанков вместо явного "нет ответа" неприемлем. Реализация — см. раздел "Этап 6.1" ниже.
> - 📋 `section_number`/`section_title` в ответе `/search` (зафиксировано 2026-06-25, не реализовано). `SearchResultChunk` сейчас возвращает `source_title` ("Трудовой кодекс РФ") и `synthetic_title` (LLM-заголовок), но не номер и заголовок статьи из структуры документа (`section_number = "128"`, `section_title = "Отпуска без сохранения заработной платы"`). Без этих полей Agent Service не может процитировать конкретную статью ("согласно статье 128 ТК РФ") — вынужден либо угадывать по тексту, либо давать ссылку только на уровне документа. Поля вычисляются при препроцессинге (`app/ingestion/preprocess.py`) и хранятся в `Chunk`, но не попадают в `build_chunk_payload` (`app/vectorstore/qdrant_client.py`) → не хранятся в Qdrant → не отдаются API. Реализация: (1) добавить `section_number: str | None` и `section_title: str` в `build_chunk_metadata`/`build_chunk_payload` → Qdrant payload; (2) добавить те же поля в `ChunkMetadata` (`app/models/metadata.py`) и `SearchResultChunk` (`app/models/schemas.py`); (3) обновить маппинг из payload в `SearchResultChunk` в поисковом пайплайне. Требует переиндексации уже загруженных документов (не удаления коллекции — повторный `/ingest` по workflow Этапа 7 заменит чанки на месте). См. детали в разделе "Этап 7" ниже.
> - 📋 Этап 13 — Гранулярное обновление статей нормативных актов (запланировано 2026-06-23, не реализовано). Сведено из двух черновиков ТЗ (`meta.md`/`upd.md`) для интеграции с будущим отдельным сервисом мониторинга изменений законодательства (вне рамок этого репозитория) — он будет точечно обновлять статьи ТК РФ/федеральных законов через API. Черновики консолидированы в этот файл и удалены из репозитория, как и документы Этапа 12. См. раздел "Этап 13" ниже, новую строку раздела 0.1 и новые поля схемы метаданных в разделе 3.
...
### Этап 6.1 — Reranker как арбитр релевантности: сигнал "нет ответа" 📋 Запланировано, не реализовано
...
### Этап 13 — Гранулярное обновление статей нормативных актов 📋 Запланировано, не реализовано
...
**Планируемые поля (Этап 13, не реализовано)** — для гранулярного обновления статей:
...
```python
{
    ...
    "effective_until": None,              # дата, когда эту редакцию сменила следующая; None — текущая. Данные на будущее для запроса "на дату X" (не реализовано)
